...
/**
 * M/Text client API example: Get the content of a specified document and show
 * its content.
 **/
public class GetClassicDocumentContent {
	/**
	 * If the specified document is available -> get the document content and show
	 * it
	 * 
	...
	public static void main(String[] args) {
		...
		MUtil.checkArguments(args, 4, GetClassicDocumentContent.class, "<username> <password> <folder path> <document name>");
		...
		try {
			...
			try {
				job.begin();
				// open text document
				persTextDoc = job.openTextDocument(fullQualifiedDocumentName, DocumentAccessMode.SHARED_READ_ONLY, null);
				...
				while (it.hasNext()) {
					...
					if (reason instanceof DocumentOpenException) {
						System.out.println("Error opening document " + documentName);
			...
			try {
				...
				while (lineIter.hasNext()) {
					...
					if (docLine instanceof TextLine) {
						...
						System.out.println("TextLine: " + ((TextLine) docLine).getText());
					}
					else {
						...
						// get the model name
						System.out.println("ModelLine: " + modelLine.getModelName());
						// apppend the arguments to the string remindModelLine
						for (int i = 0; i < modelParams.length; i++) {
							System.out.println("        Parameter" + (i + 1) + ": " + modelParams[i]);
						}
					}
				}
				// close the document
			...
			// M/Text exception occured
				...
				System.out.println("Can't read the document content from document '" + documentName + "'!");
				me.printStackTrace();
			}
			catch (Exception e) {
				e.printStackTrace();
